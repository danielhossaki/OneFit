...
if ($idUsuario <= 0) {
    preferencias_responder(false, onefitTraduzir('Sua sessão expirou. Entre novamente.'), 401);
}
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Allow: POST');
    preferencias_responder(false, onefitTraduzir('Método não permitido.'), 405);
}
$token = $_POST['csrf_token'] ?? null;
if (!is_string($token) || $token === '' || !hash_equals($_SESSION['csrf_token'] ?? '', $token)) {
    preferencias_responder(false, onefitTraduzir('Sua sessão expirou. Atualize a página e tente novamente.'), 403);
}
...
if (!is_string($chave) || !is_string($valor)) {
    preferencias_responder(false, onefitTraduzir('Preferência inválida.'), 422);
}
...
try {
    ...
    if ($chave === 'tema') {
        if (!in_array($valor, ['light', 'dark', 'system'], true)) {
            preferencias_responder(false, onefitTraduzir('Tema inválido.'), 422);
        }
        $stmt = $conn->prepare(
    ...
    $stmt->execute();
    $stmt->close();
    preferencias_responder(true, onefitTraduzir('Preferência salva.'));
} catch (Throwable $erro) {
    error_log('Falha ao salvar preferência do painel ONE FIT. Código: ' . $erro->getCode());
    preferencias_responder(false, onefitTraduzir('Não foi possível salvar. Tente novamente.'), 500);
}
