...
if ($acao === 'update') {
    ...
    $stmt = $conn->prepare('UPDATE cadastro_planos SET nome=?, valor=?, duracao_dias=?, status=?, descricao=? WHERE id_plano=?');
    $stmt->bind_param('sdissi', $nome, $valor, $dias, $status, $descricao, $id);
    $stmt->execute();
    $stmt->close();
...
$stmt = $conn->prepare('INSERT INTO cadastro_planos (nome, valor, duracao_dias, status, descricao) VALUES (?, ?, ?, ?, ?)');
$stmt->bind_param('sdiss', $nome, $valor, $dias, $status, $descricao);
$stmt->execute();
$stmt->close();
