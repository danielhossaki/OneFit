...
if (empty($_SESSION['id_usuario']) || ($_SESSION['tipo_usuario'] ?? '') !== 'aluno') {
    ...
    echo json_encode(['error' => onefitTraduzir('Entre como aluno para acessar o treino.')]);
    exit;
}
...
if (!is_string($token) || $token === '' || !hash_equals($_SESSION['csrf_token'] ?? '', $token)) {
    http_response_code(403);
    echo json_encode(['error' => onefitTraduzir('Sua sessão expirou. Atualize a página.')]);
    exit;
}
try {
    ...
    echo json_encode(['error' => onefitTraduzir($erro->getMessage())]);
} catch (Throwable $erro) {
    error_log('ONE FIT treino: código ' . $erro->getCode());
    http_response_code(500);
    echo json_encode(['error' => onefitTraduzir('Não foi possível salvar o treino. Tente novamente.')]);
}
